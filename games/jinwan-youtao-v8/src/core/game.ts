...
import type {PreviewState,Command,Department,Guest,Room,LogCategory,Booking} from '../state/types';
...
export function newGame():PreviewState {
 ...
 initDevelopment(s);s.guests.forEach(g=>initGuest(s,g));arrival(s);prepareMorning(s);log(s,'部门','Hyatt Place 正式开业。4× 已开放；关闭面板后时间继续。');updateUsage(s);return s;
}
function arrival(s:PreviewState,booking?:Booking){const g=s.game!,r=random(s),name=['陈','林','何','张','周','王','李','赵'][Math.floor(random(s)*8)]+'先生';
 ...
 attachProfile(s,q,booking);initGuest(s,q);q.goh=q.tier==='Globalist'&&random(s)<.12;q.sua=!!booking?.sua;cue(s,q,'arrival');s.guests.push(q);g.arrivals++;log(s,'入住',`${q.name} · ${q.tier} · ${q.source} 到店，计划 ${q.stayLength} 晚。`,'facility-lobby');
}
...
function settle(s:PreviewState){const g=s.game!,rs=rooms(s);let revenue=0,nights=0;
 for(const guest of s.guests)if(guest.roomId){const amount=guest.rate??g.price;revenue+=amount;guest.spend=(guest.spend??0)+amount;if(guest.source==='平台'){const commission=Math.round(amount*.15);s.metrics.cash-=commission;g.expense+=commission;}nights++;}income(s,revenue);g.nights=nights;
 for(const guest of queue(s)){loseBooking(s,guest);depart(s,guest);}
...
export function advanceGame(s:PreviewState,minutes:number){const g=s.game;if(!g||g.paused)return;
 ...
 for(let t=0;t<minutes&&!g.paused;t++){
  g.minute++;tickConstruction(s);tickWorkforce(s,c=>execute(s,c),n=>pay(s,n));bookingArrivals(s,b=>arrival(s,b));advanceDevelopment(s);const rs=rooms(s);g.roomMinutes+=rs.length;g.soldMinutes+=rs.filter(r=>r.status==='occupied').length;g.closedMinutes+=rs.filter(r=>['dirty','cleaning','maintenance'].includes(r.status)).length;
  s.atmosphere=g.minute<1020?'day':g.minute<1170?'dusk':'night';
  for(const r of rs){if(r.timer&&!r.construction&&!staffAssigned(s,r.id)&&--r.timer<=0){r.timer=undefined;r.status='available';progress(s,'service');log(s,'房态',`${r.number} 已整理完毕，可重新出售。`,r.id);}

  }
  queue(s).forEach((v,i)=>{const m=v.movement;if(m&&!m.steps.length&&v.floorId==='floor-lobby'){const x=-3.7+(i%7)*.9,z=1.55+Math.floor(i/7)*.28;if(Math.hypot(m.position.x-x,m.position.z-z)>.15)m.steps=[{x,z,level:0,phase:'public'}];}});
  for(const guest of [...s.guests]){
   ...
   if(!guest.roomId){guest.patience=(guest.patience??100)-1;if(guest.patience<=0){loseBooking(s,guest);depart(s,guest);g.lost++;g.complaints++;reputation(s,-1);log(s,'客诉',`${guest.name} 等待过久离店，失去一笔预订。`,'facility-lobby');}continue;}
   ...
   if((guest.checkoutDay??99)<=g.day&&g.minute>=dueMinute&&!guest.movement!.steps.length){
    ...
    guestStory(s,guest);checkoutMemory(s,guest,effects);r.extraBed=false;r.status='dirty';r.guestId=undefined;r.nightsLeft=0;g.memory[guest.name]=(guest.satisfaction??90)>=80?Math.min(5,(g.memory[guest.name]??0)+1):0;guest.roomId=undefined;depart(s,guest);log(s,'入住',`${guest.name} 退房，${r.number} 等待 Housekeeping 翻房。`,r.id);
   }
  }
  if(g.minute>=g.nextArrival&&g.minute<1260){if(queue(s).length<8)arrival(s);g.nextArrival=g.minute+Math.max(35,Math.round(780/walkinEstimate(s)*(.7+random(s)*.6)*(g.operations?.event==='flights'?(g.minute<1080?1.8:.45):1)));}
...
export function execute(s:PreviewState,c:Command){const g=s.game;if(!g)return;if(operationsCommand(s,c)||developmentCommand(s,c))return;const e=c.id?s.entities[c.id]:undefined;const r=e?.kind==='room'?e:null;
 switch(c.type){
 case 'checkin':{const guest=queue(s).find(a=>a.id===c.id),room=s.entities[c.roomId??''];if(!guest||room?.kind!=='room'||!(room.status==='available'||room.status==='reserved'&&guest.tier==='Globalist'&&(!room.suaBookingId||room.suaBookingId===guest.reservationId))){g.notice='住客或房态已变化，请重新选择。';break;}
  ...
  const booking=g.operations?.bookings.find(b=>b.id===guest.reservationId);if(booking)booking.status='checkedin';room.suaBookingId=undefined;room.status='occupied';room.guestId=guest.id;room.nightsLeft=guest.stayLength??2;home(s,guest);cue(s,guest,guest.denied?'denied':'checkin');progress(s,'arrivals');log(s,'入住',`${guest.name} 入住 ${room.number} · ${room.nightsLeft} 晚 · ¥${guest.rate}/晚${guest.upgrades?'，会员升套':''}。`,room.id);break;}
 case 'reject':{const guest=queue(s).find(a=>a.id===c.id);if(guest){loseBooking(s,guest);depart(s,guest);cue(s,guest,'denied');g.lost++;log(s,'入住',`已为 ${guest.name} 婉拒本次入住。`,'facility-lobby');}break;}
 case 'clean':if(r?.status==='dirty'&&pay(s,90)){r.status='cleaning';r.timer=30;log(s,'房态',`${r.number} 开始清洁，约 30 游戏分钟。`,r.id);}break;
 case 'repair':if(r?.status==='maintenance'&&!r.construction&&!r.timer&&!staffAssigned(s,r.id)&&pay(s,180)){r.timer=40;log(s,'房态',`${r.number} 开始维修。`,r.id);}else if(e?.kind==='facility'&&pay(s,200)){e.maintenance=100;log(s,'房态',`${e.name} 维护完成。`,e.id);}break;
 case 'configure-room':{if(r?.status!=='unbuilt'||s.floors.find(f=>f.id===r.floorId)?.construction){g.notice='楼层施工尚未完成。';break;}const [category,bed]=String(c.value).split(':');if(!Object.hasOwn(ROOM_TIERS,category)||!['king','twin'].includes(bed))break;const choice=category as keyof typeof ROOM_TIERS;if(!pay(s,ROOM_TIERS[choice].cost))break;r.category=choice;r.bed=bed as 'king'|'twin';r.type=choice==='suite'||choice==='premium'?'suite':r.bed;r.status='available';r.level=1;upgradeFeedback(s,r.id);log(s,'升级',`${r.number} 已设置为 ${ROOM_TIERS[choice].name} · ${bed==='twin'?'双床':'大床'}。`,r.id);break;}
 case 'upgrade':if(r?.status==='available'&&(r.level??1)<5&&pay(s,roomUpgradeCost(r.level??1))){r.construction={remaining:90,total:90,targetLevel:(r.level??1)+1};r.status='maintenance';upgradeFeedback(s,r.id);progress(s,'upgrade');log(s,'升级',`${r.number} 封闭装修：90 分钟后升级竣工。`,r.id);}break;
 case 'reserve':if(r?.status==='available'&&standardSuite(r)){r.status='reserved';log(s,'房态',`${r.number} 预留给 Globalist / SUA。`,r.id);}break;
 case 'release':if(r?.status==='reserved'&&!r.suaBookingId){r.status='available';log(s,'房态',`${r.number} 已释放预留。`,r.id);}break;
 ...
 case 'expand':{const guestFloors=s.floors.filter(f=>f.role==='guest');if(!pay(s,10000+5000*(guestFloors.length-3)))break;const no=guestFloors.length+2,floor={id:'floor-'+no,number:no,label:no+'F',name:'客房',role:'guest' as const,entityIds:[] as string[],construction:{remaining:240,total:240}};for(let c=1;c<=3;c++){const number=String(no*100+c),id='room-'+number;floor.entityIds.push(id);s.entities[id]={id,kind:'room',floorId:floor.id,number,type:'king',status:'unbuilt',nightsLeft:0,level:1};}shiftMovementFloors(s,2+guestFloors.length);s.floors.splice(2+guestFloors.length,0,floor);s.floors.forEach((f,i)=>{f.number=i;f.label=f.role==='lobby'?'L':f.role==='rooftop'?'RF':i+'F';});g.level++;log(s,'升级',`${floor.label} 客房层施工开始：4 小时后交付 3 个空位，施工期间不可配置。`,floor.entityIds[0]);break;}
