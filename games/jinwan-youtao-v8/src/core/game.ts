import type {PreviewState,Command,Department,Guest,Room,LogCategory,Task} from '../state/types';
...
export function demand(s:Readonly<PreviewState>){const g=s.game!,weekend=(g.day-1)%7>=4;return (g.positioning==='business'?(weekend?.75:1.35):g.positioning==='resort'?(weekend?1.6:.85):weekend?1.25:1.1)*(g.weather==='rain'?.88:1)*Math.max(.45,Math.min(1.5,650/g.price));}
...
function income(s:PreviewState,n:number){n=Math.round(n);s.metrics.cash+=n;s.game!.revenue+=n;}
...
function progress(s:PreviewState,id:string,n=1){const t=s.game!.tasks.find(t=>t.id===id);if(t)t.progress=Math.min(t.goal,t.progress+n);}
function tasks(day:number):Task[]{return [
 {id:'arrivals',title:`接待 ${day===1?2:3} 位住客`,goal:day===1?2:3,progress:0,reward:500,claimed:false,target:'front'},
 {id:'service',title:'完成 2 次清洁或维修',goal:2,progress:0,reward:400,claimed:false,target:'hotel'},
 {id:day===1?'stock':'delegate',title:day===1?'补充一次早餐库存':'委派一次部门 SOP',goal:1,progress:0,reward:400,claimed:false,target:'operations'}];}
export function newGame():PreviewState {
 ...
 arrival(s);log(s,'部门','Hyatt Place 正式开业。4× 已开放；关闭面板后时间继续。');updateUsage(s);return s;
}
function arrival(s:PreviewState){const g=s.game!,r=random(s),name=['陈','林','何','张','周','王','李','赵'][Math.floor(random(s)*8)]+'先生';
 ...
 const q:Guest={id:'guest-'+g.nextId++,name,tier,floorId:'floor-lobby',thought:tier==='Globalist'?'今晚有套吗？':'想住 '+stay+' 晚',color:0x36566a,route:[-2.5,2.5],z:1.7,segment:holiday?'度假':stay>=5?'长住':'商务',stayLength:stay,patience:100+(g.managers.front?50:0),satisfaction:90+(g.memory[name]??0)};
...
function settle(s:PreviewState){const g=s.game!,rs=rooms(s);let revenue=0,nights=0;
 ...
 g.reports.push({day:g.day,revenue:g.revenue,expense:g.expense,adr,occupancy,revpar:Math.round(revenue/Math.max(1,rs.length)),upgrades:g.upgrades,complaints:g.complaints,lost:Math.round(g.closedMinutes/Math.max(1,g.roomMinutes)*100),recommendation:rec});
...
function nextDay(s:PreviewState){const g=s.game!;g.day++;g.minute=420;g.nextArrival=450;g.nextEvent=600;g.weather=random(s)<.25?'rain':'sunny';g.revenue=g.expense=g.nights=g.arrivals=g.upgrades=g.complaints=g.lost=g.repLoss=g.roomMinutes=g.soldMinutes=g.closedMinutes=0;g.reportOpen=false;g.paused=false;g.tasks=tasks(g.day);for(const r of rooms(s))if(r.guestId){const guest=s.guests.find(a=>a.id===r.guestId);r.nightsLeft=Math.max(0,(guest?.checkoutDay??g.day)-g.day);}if(g.managers.revenue)g.price=Math.round((demand(s)>1.1?750:590)*(1+g.level*.04));log(s,'部门',`${weekday(g.day)} 开始。${g.weather==='rain'?'今天有雨。':''}预计需求 ${Math.round(demand(s)*100)}%。`);}
export function advanceGame(s:PreviewState,minutes:number){const g=s.game;if(!g||g.paused)return;
 for(let t=0;t<minutes&&!g.paused;t++){
  g.minute++;const rs=rooms(s);g.roomMinutes+=rs.length;g.soldMinutes+=rs.filter(r=>r.status==='occupied').length;g.closedMinutes+=rs.filter(r=>['dirty','cleaning','maintenance'].includes(r.status)).length;
  ...
  for(const r of rs){if(r.timer&&--r.timer<=0){r.timer=undefined;r.status='available';progress(s,'service');log(s,'房态',`${r.number} 已整理完毕，可重新出售。`,r.id);}
   if(r.status==='dirty'&&g.managers.house){r.status='cleaning';r.timer=20;progress(s,'delegate');log(s,'部门',`客房主管安排清洁 ${r.number}。`,r.id);}
   if(r.status==='maintenance'&&!r.timer&&g.managers.engineering&&pay(s,100)){r.timer=30;progress(s,'delegate');log(s,'部门',`工程主管接管 ${r.number} 的维修。`,r.id);}
  }
  for(const guest of [...s.guests]){
   ...
   if(!guest.visitUntil&&g.minute%45===0){let role=g.minute<630?'breakfast':g.minute>=1020&&g.minute<1260?'club':g.minute>=1260?'rooftop':'gym';if(role==='club'&&guest.tier==='普通客')role='rooftop';const f=s.entities['facility-'+role];if(f?.kind==='facility'&&random(s)<.6){
    ...
    if(role==='breakfast'||role==='club'){const key=role==='breakfast'?'stock':'clubStock';if(g[key]>0){g[key]--;guest.satisfaction=Math.min(100,(guest.satisfaction??90)+1);if(role==='club'&&guest.tier!=='Globalist')income(s,80);}else{guest.thought='怎么没东西吃了';guest.satisfaction=(guest.satisfaction??90)-5;if(g.minute%90===0){g.complaints++;reputation(s,-1);log(s,'客诉',`${f.name} 缺货，${guest.name} 不满意。`,f.id);}}}else if(role==='rooftop')income(s,45);
  ...
  if(g.managers.fnb&&g.minute%30===0){for(const key of ['stock','clubStock'] as const)if(g[key]<20&&pay(s,240)){g[key]+=40;progress(s,'delegate');log(s,'部门','餐饮主管已自动补货。','facility-breakfast');}}
  if(g.minute>=g.nextArrival&&g.minute<1260){if(queue(s).length<6)arrival(s);g.nextArrival=g.minute+Math.max(22,Math.round((35+random(s)*28)/demand(s)));}
...
export function execute(s:PreviewState,c:Command){const g=s.game;if(!g)return;const e=c.id?s.entities[c.id]:undefined;const r=e?.kind==='room'?e:null;
 switch(c.type){
 case 'checkin':{const guest=queue(s).find(a=>a.id===c.id),room=s.entities[c.roomId??''];if(!guest||room?.kind!=='room'||!(room.status==='available'||room.status==='reserved'&&guest.tier==='Globalist')){g.notice='住客或房态已变化，请重新选择。';break;}
  guest.roomId=room.id;guest.checkoutDay=g.day+(guest.stayLength??2);guest.rate=Math.round(g.price*(room.type==='suite'?(guest.tier==='Globalist'?1:1.45):1)*(1+(room.level??1)*.035));guest.upgrades=room.type==='suite'&&guest.tier==='Globalist';if(guest.upgrades){g.upgrades++;reputation(s,1);}else if(guest.tier==='Globalist'&&rooms(s).some(a=>a.type==='suite'&&a.status==='available'))reputation(s,-1);
 ...
 case 'upgrade':if(r?.status==='available'&&(r.level??1)<3&&pay(s,2500*(r.level??1))){r.level=(r.level??1)+1;log(s,'升级',`${r.number} 装修至 ${r.level} 级，提高房价。`,r.id);}break;
