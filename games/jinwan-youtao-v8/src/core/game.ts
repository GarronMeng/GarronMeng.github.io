...
export function execute(s:PreviewState,c:Command){const g=s.game;if(!g)return;if(developmentCommand(s,c))return;const e=c.id?s.entities[c.id]:undefined;const r=e?.kind==='room'?e:null;
 ...
 case 'checkin':{const guest=queue(s).find(a=>a.id===c.id),room=s.entities[c.roomId??''];if(!guest||room?.kind!=='room'||!(room.status==='available'||room.status==='reserved'&&guest.tier==='Globalist')){g.notice='住客或房态已变化，请重新选择。';break;}
  guest.roomId=room.id;guest.checkoutDay=g.day+(guest.stayLength??2);guest.rate=roomRate(g.price,room,guest.tier==='Globalist');guest.upgrades=room.type==='suite'&&guest.tier==='Globalist';guest.denied=guest.tier==='Globalist'&&room.type!=='suite';if(guest.upgrades){g.upgrades++;progress(s,'vip');reputation(s,1);}else if(guest.tier==='Globalist'&&rooms(s).some(a=>a.type==='suite'&&a.status==='available'))reputation(s,-1);
 ...
 case 'upgrade':if(r?.status==='available'&&(r.level??1)<5&&pay(s,roomUpgradeCost(r.level??1))){r.level=(r.level??1)+1;progress(s,'upgrade');log(s,'升级',`${r.number} 装修至 ${r.level} 级，提高房价。`,r.id);}break;
