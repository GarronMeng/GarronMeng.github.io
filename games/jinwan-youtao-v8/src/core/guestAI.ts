...
export function publicLoad(s:Readonly<PreviewState>,id:string){return s.guests.filter(g=>!g.staff&&!g.departing&&!g.waitingFor&&(g.movement?.destination===id&&(g.movement.steps.length>0||g.movement.position.phase==='public'))).length;}
...
export function destinationWeights(s:Readonly<PreviewState>,g:Guest){const minute=s.game!.minute,h=minute/60,p=PERSONAS[g.persona??'chill'],weights:{id:string;weight:number}[]=[{id:g.roomId!,weight:h>=22||h<7?25:3}];
 for(const f of Object.values(s.entities)){if(f.kind!=='facility'||f.construction)continue;
  let weight=0;
  ...
  const load=publicLoad(s,f.id);weight*=load+s.guests.filter(v=>v.waitingFor===f.id).length>=f.capacity+4?0:Math.max(.1,1-load/Math.max(1,f.capacity));
...
function seated(s:PreviewState,id:string,except:string){return s.guests.filter(v=>v.id!==except&&!v.staff&&!v.waitingFor&&v.movement?.destination===id&&!v.movement.steps.length&&v.movement.position.phase==='public').length;}
function waitAt(s:PreviewState,g:Guest,id:string){g.waitingFor=id;g.waitSince=hotelTime(s);const m=g.movement!;m.steps=[{x:6.2,z:2.05,level:m.position.level,phase:'public'}];}
