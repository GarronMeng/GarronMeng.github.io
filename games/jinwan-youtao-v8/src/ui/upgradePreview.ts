...
export function roomPreview(s:Readonly<PreviewState>,r:Room){const level=r.level??1;if(level>=5)return '<p>装修已满级</p>';const g=s.game!,next={...r,level:level+1},cost=roomUpgradeCost(level),before=roomRate(g.price,r),after=roomRate(g.price,next),delta=after-before;
 ...
 return `<details class="upgrade-preview"><summary>装修 Lv.${level} → ${level+1} · ${money(cost)}</summary>${compare('新客每晚房价',before,after,' 元')}${standardSuite(r)?`<p>会员免费升套价：${money(roomRate(g.price,r,true))} → ${money(roomRate(g.price,next,true))}</p>`:''}<p>每售出一晚多收 ${money(low)}${low!==delta?'–'+money(delta):''}；约 ${Math.ceil(cost/delta)}${low!==delta?'–'+Math.ceil(cost/low):''} 个售出房晚收回装修费。</p><p>按${g.reports.length?'最近一天':'当前'}入住率 ${Math.round(occupancy)}%、${standardSuite(r)?'普通付费客':'当前挂牌价'}估算：每天多收 ${money(daily)}${daily>0?'，约 '+Math.ceil(cost/daily)+' 天回本':'，暂无法估算回本天数'}。</p><small>已确认预订和已入住订单价格不变；预估假设房价、入住率保持不变。</small></details>`;
}
export function managerPreview(s:Readonly<PreviewState>,id:Department,name:string,base:number){const g=s.game!,level=g.managers[id],next=Math.min(3,level+1),cost=level?4500*level:3800;
 ...
 if(id==='house'){effect=compare('每间清洁用时',houseMinutes(level),houseMinutes(next),' 分钟');note=level?'增加一位客房员工，每间现场清洁少用 '+(houseMinutes(level)-houseMinutes(next))+' 分钟；另需实际走路时间。':'自动清洁脏房，免手动清洁 ¥90/间；每日工资 ¥180，两次清洁抵消工资。';}
