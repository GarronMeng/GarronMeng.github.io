...
test('入住、午夜唯一结算、多晚跨日、扩建与主管连续运行',()=>{
 const s=playableGame(),g=s.game!,q=queue(s)[0],r=rooms(s).find(r=>r.status==='available')!;
 ...
 for(let day=0;day<7;day++){advanceGame(s,1600);assert.equal(g.reportOpen,true);const cash=s.metrics.cash;advanceGame(s,1600);assert.equal(s.metrics.cash,cash);execute(s,{type:'continue'});execute(s,{type:'brief-start'});}
 ...
 s.metrics.cash=200000;for(let i=0;i<4;i++)execute(s,{type:'expand'});assert.ok(s.floors.some(f=>f.label==='8F'&&f.role==='guest'));assert.equal(roomSlots(s).length,21);assert.equal(rooms(s).length,9);
...
test('解除21间限制、投资活动结算、领奖幂等及旧存档兼容',async()=>{
 ...
 const s=playableGame(),g=s.game!;s.metrics.cash=1000000;
 for(let i=0;i<9;i++)execute(s,{type:'expand'});
 assert.equal(roomSlots(s).length,36);for(let i=0;i<240;i++)tickConstruction(s);for(const r of roomSlots(s).filter(r=>r.status==='unbuilt'))execute(s,{type:'configure-room',id:r.id,value:'standard:king'});assert.equal(rooms(s).length,36);assert.equal(new Set(s.floors.map(f=>f.id)).size,s.floors.length);
 const f=s.entities['facility-gym'];assert.equal(f.kind,'facility');if(f.kind!=='facility')throw Error();const capacity=f.capacity;
 execute(s,{type:'invest',id:f.id});for(let i=0;i<120;i++)tickConstruction(s);assert.equal(f.level,2);assert.equal(f.capacity,capacity+4);
