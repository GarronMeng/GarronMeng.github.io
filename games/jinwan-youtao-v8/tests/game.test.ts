...
test('解除21间限制、投资活动结算、领奖幂等及旧存档兼容',async()=>{
 ...
 assert.equal(roomSlots(s).length,36);for(const r of roomSlots(s).filter(r=>r.status==='unbuilt'))execute(s,{type:'configure-room',id:r.id,value:'standard:king'});assert.equal(rooms(s).length,36);assert.equal(new Set(s.floors.map(f=>f.id)).size,s.floors.length);
