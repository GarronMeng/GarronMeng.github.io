...
test('对白人格、上下文与冷却；4PM兑现并保存旧版人物',async()=>{
 assert.equal(Object.keys(PERSONAS).length,11);const s=newGame(),g=s.guests.find(g=>g.roomId)!;g.persona='points';cue(s,g,'recovery');assert.match(g.thought,/QN/);const first=g.thought;cue(s,g,'recovery');assert.notEqual(g.thought,first);
