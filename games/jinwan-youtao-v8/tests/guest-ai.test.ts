...
test('房间—走廊—电梯—公区—房间完整连续；4倍批次保留路径',()=>{
 const s=playableGame(),g=s.guests.find(g=>g.roomId)!;ensureMovement(s,g);
...
test('人格/时段/雨天/会员/容量分流，Spa接入且不强制去屋顶',()=>{
 const s=playableGame(),g=s.guests.find(g=>g.roomId)!;s.metrics.cash=100000;execute(s,{type:'build-spa'});assert.ok(s.entities['facility-spa']);
 ...
 const other=s.guests.find(v=>v.roomId&&v.id!==g.id)!;const crowded=destinationWeights(s,other).find(d=>d.id===f.id)!.weight;f.capacity=8;assert.ok(destinationWeights(s,other).find(d=>d.id===f.id)!.weight>crowded);
});
test('对白人格、上下文与冷却；4PM兑现并保存旧版人物',async()=>{
 assert.equal(Object.keys(PERSONAS).length,11);const s=playableGame(),g=s.guests.find(g=>g.roomId)!;g.persona='points';g.serviceDone=true;cue(s,g,'recovery');assert.match(g.thought,/QN/);const first=g.thought;cue(s,g,'recovery');assert.notEqual(g.thought,first);
...
test('会员晚退保留手动决策，2PM/4PM生效，新客短住占多数',async()=>{
 ...
 const s=playableGame();s.game!.minute=540;s.game!.managers.front=3;
...
test('装修预览与真实结算使用一致价格，缺货对白不夸赞用餐',async()=>{
 const {roomRate}=await import('../src/core/economy');const {speak}=await import('../src/core/guestDialogue');const s=playableGame();s.metrics.cash=100000;
 const q=queue(s)[0],r=rooms(s).find(r=>r.status==='available')!;execute(s,{type:'upgrade',id:r.id});for(let i=0;i<90;i++)tickConstruction(s);execute(s,{type:'checkin',id:q.id,roomId:r.id});assert.equal(q.rate,roomRate(s.game!.price,r,q.tier==='Globalist'));
