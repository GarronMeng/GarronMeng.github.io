...
test('新增三空位、八种配置、重复点击不扣费、尊享套房正常计价',()=>{
 for(const tier of ['standard','view','suite','premium'])for(const bed of ['king','twin']){
  const s=playableGame();s.metrics.cash=100000;execute(s,{type:'expand'});assert.equal(rooms(s).length,9);const empty=roomSlots(s).filter(r=>r.status==='unbuilt');assert.equal(empty.length,3);
  for(let i=0;i<240;i++)tickConstruction(s);const r=empty[0];execute(s,{type:'configure-room',id:r.id,value:tier+':'+bed});assert.equal(roomTier(r),tier);assert.equal(roomBed(r),bed);assert.equal(rooms(s).length,10);
