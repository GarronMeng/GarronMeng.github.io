...
function syncActors(){
  ...
  for(const item of stateActors()){
    const d=item.data;keep.add(d.id);const p=pos(item.i,item.enemy);
    let g=actors.get(d.id);
    if(!g){g=buildActor(d,item.enemy);g.position.set(item.enemy?4.75:-4.75,0,p.z);g.scale.setScalar(.72);g.userData.entering=1;actors.set(d.id,g);scene.add(g)}
    g.userData.homeX=p.x;g.userData.homeZ=p.z;g.userData.targetX=p.x;g.userData.targetZ=p.z;g.userData.dead=d.hp<=0||d.dead;g.userData.targetY=g.userData.dead?-.28:0;g.userData.kind=d.type;
  }
...
function destroy(){
  ...
  for(const g of actors.values())disposeObject(g);actors.clear();for(const b of bolts){scene?.remove(b.o);b.o.geometry?.dispose?.();b.o.material?.dispose?.()}bolts.length=0;for(const e of impacts){scene?.remove(e.o);e.o.geometry?.dispose?.();e.o.material?.dispose?.()}impacts.length=0;
  if(scene){
...
function addImpact(at,color=0xf4d47d,size=.18){if(!scene)return;const mat=new THREE.MeshBasicMaterial({color,transparent:true,opacity:.75,side:THREE.DoubleSide,depthWrite:false});const o=new THREE.Mesh(new THREE.RingGeometry(size*.48,size,18),mat);o.position.copy(at);o.rotation.x=-Math.PI/2;scene.add(o);impacts.push({o,t:0})}
function markHit(g,power=1){if(g)g.userData.hit=Math.max(g.userData.hit||0,power)}
function spawnBolt({src,tgt,kind,good,srcType}){if(!API.enabled||!scene)return;const a=actors.get(src),b=actors.get(tgt);if(!a||!b)return;const melee=kind==='melee'||kind==='enemyMelee';if(melee){a.userData.lunge={target:b,t:0};a.userData.swing=1;setTimeout(()=>{markHit(b,1);if(scene&&b.parent)addImpact(b.position.clone().add(new THREE.Vector3(0,.05,0)),kind==='enemyMelee'?0xff727d:0xf4d47d,srcType==='boss'?.34:.2)},130);if(srcType==='boss')setTimeout(()=>bossShock(a),80);return}const magic=kind==='magic'||kind==='enemyMagic',heal=kind==='heal';const color=heal?0x6ce0a9:magic?(kind==='enemyMagic'?0xd081ff:0xb983ff):kind==='enemyShot'?0xff817d:0xf4d47d;const orb=addMesh(scene,new THREE.SphereGeometry((magic||heal)?.09:.052,7,5),new THREE.MeshBasicMaterial({color}));orb.position.copy(a.position).add(new THREE.Vector3(0,.9,0));bolts.push({o:orb,a:orb.position.clone(),target:b,t:0,color,heal});a.userData.swing=1}
function bossShock(g){if(!scene||!g)return;for(let i=0;i<3;i++)setTimeout(()=>addImpact(g.position.clone().add(new THREE.Vector3(0,.03,0)),0xba5b82,.38+i*.14),i*70)}
function mergeActors({src,tgt}){const a=actors.get(src),b=actors.get(tgt);if(!a||!b)return;a.userData.merge={target:b,t:0};for(let i=0;i<4;i++)setTimeout(()=>addImpact(b.position.clone().add(new THREE.Vector3(0,.04,0)),i%2?0xf2cf72:0x78baff,.18+i*.04),180+i*55)}
function animate(ts){
  ...
  let ix=0;const battleSpeed=window.__RW_GET_STATE?.()?.battleSpeed||1;
  for(const g of actors.values()){const baseScale=g.userData.kind==='boss'?1.42:g.userData.kind==='brute'?1.16:1;let desiredX=g.userData.homeX??g.userData.targetX??0,desiredZ=g.userData.homeZ??g.userData.targetZ??0;if(g.userData.lunge){const L=g.userData.lunge,target=L.target;if(target?.parent){L.t+=.075*battleSpeed;const q=Math.min(1,L.t),push=Math.sin(Math.PI*q),contact=target.position.x+(g.userData.enemy?.62:-.62);desiredX=(g.userData.homeX??g.position.x)+(contact-(g.userData.homeX??g.position.x))*push;desiredZ=(g.userData.homeZ??g.position.z)+(target.position.z-(g.userData.homeZ??g.position.z))*push*.55;if(q>.32&&q<.68)g.userData.swing=Math.max(g.userData.swing||0,.9);if(q>=1)g.userData.lunge=null}else g.userData.lunge=null}if(g.userData.merge){const M=g.userData.merge,target=M.target;if(target?.parent){M.t+=.07;const q=Math.min(1,M.t);desiredX=(g.userData.homeX??g.position.x)+(target.position.x-(g.userData.homeX??g.position.x))*q;desiredZ=(g.userData.homeZ??g.position.z)+(target.position.z-(g.userData.homeZ??g.position.z))*q;if(q>=1)g.userData.merge=null}else g.userData.merge=null}g.position.x+=(desiredX-g.position.x)*(g.userData.entering?.10:.2);g.position.z+=(desiredZ-g.position.z)*(g.userData.entering?.10:.18);if(g.userData.entering)g.userData.entering=Math.max(0,g.userData.entering-.035);const bob=g.userData.dead?0:Math.sin(t*2.5+ix*.8)*.03;g.position.y+=((g.userData.targetY??0)+bob-g.position.y)*.18;let targetScale=baseScale;if(g.userData.entering)targetScale*=.92;if(g.userData.hit){targetScale*=1+.07*g.userData.hit;g.userData.hit=Math.max(0,g.userData.hit-.13)}if(g.userData.dead){g.rotation.z+=(g.userData.enemy?-1.2:1.2-g.rotation.z)*.09;targetScale*=.62}else g.rotation.z*=.82;g.scale.lerp(new THREE.Vector3(targetScale,targetScale,targetScale),.15);if(g.userData.starRing)g.userData.starRing.rotation.z=t*1.15;const w=g.userData.weapon;if(w){const sw=g.userData.swing||0;if(sw>0){w.rotation.z=Math.sin((1-sw)*Math.PI)*-.95;g.userData.swing=Math.max(0,sw-.14*battleSpeed)}else w.rotation.z*=.76}ix++}
