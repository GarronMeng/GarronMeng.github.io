...
let portal=null, portalCore=null, motes=null, floorGlow=null, shake=0;
...
function pos(i,enemy,battle=false){const row=Math.floor(i/2),col=i%2,z=[-1.18,0,1.18][row]??1.18;if(battle)return{x:enemy?(col?1.85:2.62):(col?-1.85:-2.62),z};return{x:enemy?(col?2.15:3.15):(col?-2.15:-3.15),z}}
...
function ensureRenderer(){if(renderer)return;canvas=document.createElement('canvas');canvas.id='three-stage';canvas.setAttribute('aria-hidden','true');renderer=new THREE.WebGLRenderer({canvas,antialias:true,alpha:true,powerPreference:'high-performance'});renderer.setPixelRatio(Math.min(window.devicePixelRatio||1,1.5));renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.setClearColor(0x000000,0);buildWorld();timer.reset()}
...
function destroy(){pause();ro?.disconnect();ro=null;for(const g of actors.values())disposeObject(g);actors.clear();for(const b of bolts){scene?.remove(b.o);b.o.geometry?.dispose?.();b.o.material?.dispose?.()}bolts.length=0;for(const e of impacts){scene?.remove(e.o);e.o.geometry?.dispose?.();e.o.material?.dispose?.()}impacts.length=0;if(scene)scene.traverse(o=>{o.geometry?.dispose?.();if(o.material)(Array.isArray(o.material)?o.material:[o.material]).forEach(m=>m.dispose?.())});renderer?.dispose?.();timer.dispose?.();renderer=null;scene=null;camera=null;canvas=null;stage=null}
...
function spawnBolt({src,tgt,kind,good,srcType}){
  if(!API.enabled||!scene)return;const a=actors.get(src),b=actors.get(tgt);if(!a||!b)return;
...
function animate(ts){
  if(!renderer||!scene||!API.enabled)return;raf=requestAnimationFrame(animate);timer.update(ts);const t=timer.getElapsed(),battleSpeed=window.__RW_GET_STATE?.()?.battleSpeed||1;
  ...
  for(const g of actors.values()){
    ...
    if(g.userData.dead){const fall=g.userData.enemy?-1.2:1.2;g.rotation.z+=(fall-g.rotation.z)*.09;targetScale*=.62}else g.rotation.z*=.82;g.scale.lerp(new THREE.Vector3(targetScale,targetScale,targetScale),.15);
