...
function updateHudLabels(){if(!stage||!camera)return;const w=stage.clientWidth,h=stage.clientHeight,items=[];for(const [id,g] of actors){const el=document.querySelector(`.combatant[data-cid="${id}"]`);if(!el)continue;const p=g.localToWorld(new THREE.Vector3(0,1.78,0)).project(camera);if(p.z>1){el.style.visibility='hidden';continue}let x=(p.x*.5+.5)*w,y=(-p.y*.5+.5)*h;x=Math.max(38,Math.min(w-38,x));y=Math.max(48,Math.min(h-58,y));items.push({el,x,y})}items.sort((a,b)=>b.y-a.y);const placed=[];for(const it of items){let y=it.y;for(const p of placed)if(Math.abs(it.x-p.x)<72&&Math.abs(y-p.y)<20)y=p.y-20;y=Math.max(46,Math.min(h-58,y));it.el.style.visibility='visible';it.el.style.left=`${it.x}px`;it.el.style.top=`${y}px`;it.el.style.transform='translate(-50%,-100%)';placed.push({x:it.x,y})}}
function animate(ts){
