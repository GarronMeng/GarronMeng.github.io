...
export function createVFXSystem({scene,actors,timer,shake}){
  ...
  function cast(detail){const a=actors.get(detail.src),b=actors.get(detail.tgt);if(!a||!b)return;const type=detail.srcType||a.userData.kind,star=a.userData.star||1,strong=cadence(a),src=hand(a),dst=head(b),dir=dst.clone().sub(src).normalize();
    if(type==='guard'&&detail.kind==='melee'){setTimeout(()=>{if(!b.parent)return;const at=b.position.clone();pulseRing(at,0x7ebcff,strong?.34+star*.035:.22,strong?.5:.32);particles.emit(at.clone().add(new THREE.Vector3(0,.12,0)),0xa7d2ff,strong?14+star*4:5,{speed:strong?.8:.55,spread:strong?1.2:.65,up:strong?.65:.32,size:strong?.09:.055,life:strong?.5:.32});if(strong){setTimeout(()=>pulseRing(at,0xb8dcff,.46+star*.025,.5),60);shake(.055*star)}},105);return}
    ...
    if(type==='medic'&&detail.kind==='heal'){beam(src,dst,0x72efb3,strong?.052:.028,strong?.34:.23);const at=b.position.clone();pulseRing(at,0x68dfaa,.22+star*.025,.5);particles.emit(at.clone().add(new THREE.Vector3(0,.15,0)),0x7ff0bb,strong?22:10,{speed:.52,spread:.62,up:1.25,size:.08,life:.72});if(strong){const low=at.clone().add(new THREE.Vector3(0,.08,0)),high=at.clone().add(new THREE.Vector3(0,2.1,0));beam(low,high,0x9ff6c8,.035,.52);setTimeout(()=>pulseRing(at,0xc4ffda,.38,.62),85)}return}
    if(type==='rogue'&&detail.kind==='melee'){setTimeout(()=>{if(!b.parent)return;slash(b.position.clone(),0xc47dff,strong&&star>=3);particles.emit(dst,0xd69cff,strong?20+star*4:6,{speed:strong?1.2:.75,spread:strong?1.55:.7,up:.12,size:strong?.065:.045,life:strong?.36:.25});if(strong){pulseRing(b.position.clone(),0x8153c7,.24+star*.025,.38);shake(.035*star)}},85);return}
    if(type==='boss'){const at=b.position.clone();pulseRing(at,0xff5d76,strong?.55:.30,strong?.7:.38);setTimeout(()=>{if(!b.parent)return;particles.emit(head(b),0xff667a,strong?34:10,{speed:strong?1.35:.85,spread:strong?1.8:.85,up:.45,size:strong?.12:.07,life:strong?.7:.42});if(strong){for(let i=0;i<3;i++)pulseRing(at,0xa94e79,.36+i*.16,.52+i*.08);shake(.22)}else shake(.055)},strong?180:90)}
  }
