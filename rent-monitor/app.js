const state={data:null,view:'all',filters:{area:true,rent:true,direct:true,split:false},sort:'score'};
...
function statusBadges(x){
  ...
  if(x.lift===true)out.push(badge('有电梯','good'));
  else if(x.lift===null)out.push(badge('电梯待确认','warn'));
  return out.join('')
}
function strict(x){
  return x.area>=300&&x.rent<=14000&&x.direct_to_admiralty&&x.shared!==true&&x.type_verified===true&&x.mtr_verified===true&&x.mtr_walk_min<=5&&x.lift===true&&x.split_ac===true&&x.window_ac!==true
}
function passes(x){
  if(state.view==='strict'&&!strict(x))return false;
  if(state.view==='new'&&!x.is_new&&!(x.price_drop>0))return false;
  if(state.filters.area&&x.area<300)return false;
  if(state.filters.rent&&x.rent>14000)return false;
  if(state.filters.direct&&!x.direct_to_admiralty)return false;
  if(state.filters.split&&x.split_ac!==true)return false;
  return true
}
function sorted(items){return [...items].sort((a,b)=>{if(state.sort==='area')return b.area-a.area;if(state.sort==='rent')return a.rent-b.rent;if(state.sort==='fresh')return String(b.last_seen).localeCompare(String(a.last_seen));return (b.score||0)-(a.score||0)||a.rent-b.rent})}
function card(x){
  const base=state.data.baseline;const areaGain=x.area-base.area;const rentGain=x.rent-base.rent;
  return `<article class="listing">
<div class="listing-top"><div class="listing-title"><h2>${x.estate||x.title}</h2><p>${x.district}${x.unit?` · ${x.unit}`:''}${x.rooms?` · ${x.rooms}房`:''}</p></div><div class="score">${x.score||'—'}</div></div>
...
<div class="facts"><div class="fact"><span>港铁 / 金钟</span>${x.mtr_station||x.district}${x.mtr_walk_min!=null?` · 步行${x.mtr_walk_min}分钟`:''} · ${x.admiralty_route||'路线待确认'}</div><div class="fact"><span>深圳</span>${x.shenzhen_note||'一般'}</div><div class="fact"><span>住宅类型</span>${x.type_verified===true?'常规独立住宅':'待确认'}</div><div class="fact"><span>浴室</span>${x.wet_dry===true?'干湿分离':x.wet_dry===false?'非干湿分离':'待确认'}</div><div class="fact"><span>最后确认</span>${fmtDate(x.last_seen)}</div></div>
<div class="upgrade"><span>相比基安</span><strong>${rentGain>=0?'+':''}${fmtMoney(rentGain).replace('HK$-','-HK$')}/月 · ${areaGain>=0?'+':''}${areaGain}呎</strong></div>
<div class="listing-actions"><a class="open-link" href="${x.url}" target="_blank" rel="noopener">打开当前房源</a><span class="source">${x.source}</span></div>
</article>`
}
function render(){
  if(!state.data)return;const items=sorted(state.data.listings.filter(passes));
  document.querySelector('#listings').innerHTML=items.map(card).join('');document.querySelector('#empty').hidden=items.length>0;document.querySelector('#resultMeta').textContent=`${items.length} 套`;document.querySelector('#activeCount').textContent=state.data.listings.length;document.querySelector('#strictCount').textContent=state.data.listings.filter(strict).length;document.querySelector('#newCount').textContent=state.data.listings.filter(x=>x.is_new||x.price_drop>0).length;document.querySelector('#sourceCount').textContent=state.data.meta?.source_count||0;document.querySelector('#updatedAt').textContent=state.data.meta?.updated_at?`更新 ${fmtDate(state.data.meta.updated_at)}`:'等待首次刷新';document.querySelector('#refreshStatus').textContent=state.data.meta?.status==='ok'?'监控正常':state.data.meta?.status==='partial'?'部分来源异常':'等待刷新'
}
