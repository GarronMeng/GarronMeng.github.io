...
s=p.read_text()
# Keep existing HUD architecture but make it lighter and centered.
...
# Room shells read as architecture, not rounded web cards.
...
# 2.5D cutaway renderer with Hyatt-inspired material palettes and room variants.
code=r'''function drawMiniInterior(c,type,x,y,w,h,alpha,variant){
  ...
  c.fillStyle=wall;c.fillRect(x,y,w,h*.42);
  c.beginPath();c.moveTo(x,y+h*.42);c.lineTo(x+w,y+h*.42);c.lineTo(x+w*.92,y+h);c.lineTo(x+w*.08,y+h);c.closePath();c.fillStyle=floor;c.fill();
...
# Lobby becomes part of the cutaway shell.
...
# Internal version marker only; no player-facing dev copy.
s=re.sub(r"var QUEST_SYSTEM_VERSION='[^']+';","var QUEST_SYSTEM_VERSION='6.3.0';",s,count=1)
p.write_text(s)
